...
def extract_hard_clip_from_cigar(cigar):
	cigar_5 = ["0"] + re.findall(r"^(\d+)H",cigar)
	cigar_3 = ["0"] + re.findall(r"(\d+)H$",cigar)
	cigar_5_h,cigar_3_h = [0,0]
	for s5 in cigar_5:
		cigar_5_h += int(s5)
	for s3 in cigar_3:
		cigar_3_h += int(s3)
...
def convert(inputs):
	...
	if "H" in cigar: # hard-clipped reads
		# soft-clip  and hard-clip info
		s5,s3 = extract_soft_clip_from_cigar(cigar)
		h5,h3 = extract_hard_clip_from_cigar(cigar)
		sf = str(s5)+"_"+str(s3)+"_"+str(h5)+"_"+str(h3)
		# exon info
		cigar_n_l = 0
		exon_length = 0
...
7. MAPQ 
8. number of nucleotides that are soft-clipped and hard-clipped by aligner; leftSoftClip_rightSoftClip_leftHardClip_rightHardClip
9. exon count
10. exon start set
11. exon end set
12. sam flag'''
	parser = argparse.ArgumentParser(description="Function: convert sam to gpd (Hard-clip alignment, specifically for fusion transcript detection), specifically for PacBio transcriptome long-read data after the processes: 1) extract ROI by PacBio SMRTanalysis2.3 'ConsensusTools.sh CircularConsensus'; 2) classifiy ROI by PacBio SMRTanalysis2.3 'pbtranscript.py classify' with the parameter '--detect_chimera_nfl' to get full-length (FL) and non-full-length (nFL), non-chimera (NC) ROI; 3) separate the nFLNC by the script './utilities/py_isoseqcon_separate_nflnc_fasta.py' to get nFLNC_sense.fasta and nFLNC_unknown.fasta; and 4) align ROI by GMAP aligner with the parameter '-z sense_force -f samse -n 0 --split-output' for FLNC and nFLNC_sense; 4) take the *.transloc file as the input of this script",formatter_class=argparse.ArgumentDefaultsHelpFormatter)
	parser.add_argument('-i','--input',type=argparse.FileType('r'),required=True,nargs="+",help="Input: sam file(s) (Note: multiple sam file can be as inputs split by 'space'. Currently, only the sam output of GMAP aligner is tested. Only the long reads with sense strand is permissible (i.e., FLNC.fasta and nFLNC_sense.fasta).")
